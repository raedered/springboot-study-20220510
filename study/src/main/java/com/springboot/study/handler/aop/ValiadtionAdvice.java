...
/*
 * Aspect: 여러 핵심 기능에 적용될 관심사 모튤을 의미한다. Aspect에는 구체적인 기능을 구현한 Advice와 Advice가 어디(Target)에서 적용될지 결정하는
 * 			PointCut을 포함하고 있다.
 * 
 * Target: 공통 기능을 부여할 대상. 즉, 핵심 기능을 담당하는 비즈니스 로직이고, 어떤 관심사들과도 관계를 맺지 않는다.
 * 
 * Advice: 공통 기능을 담은 구현체. Advice는 Aspect가 무엇을 언제 적용할 지를 정의 하는 것.
 * 
 * PointCut: 공동 기능이 적용될 대상을 결정하는 것.
 * 
...
@Aspect
@Component
public class ValiadtionAdvice {
	
	private final Logger LOGGER = LoggerFactory.getLogger(ValiadtionAdvice.class);
